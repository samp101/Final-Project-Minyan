        ...
        <Stack sx={{position:'relative', marginTop:-15, display:'flex', flexDirection:'row', justifyContent:'space-evenly', flexWrap:'wrap', alignItems:'center', width:'98vw'}}>
                {props.shul.map((shul,ind)=>{
                    ...
                    <Box sx={{ m:3,  display:'flex',flexDirection:'column', alignItems:'flex-start', justifyContent:'flex-start', background:'white', width:'30%', border:1, borderColor:'#f6efe5', borderRadius:10, padding:5,}}>
                        <Typography sx={{fontSize:'30px', fontWeight:600, color:'#321154'}}>{shul.shul.shul_name}</Typography>
                        <Typography sx={{fontSize:'20px', color:'#321154'}}>{shul.shul.shul_address}, {shul.shul.shul_city_city}, {shul.shul.shul_country} </Typography>
                        <Typography sx={{marginTop:3, fontSize:'22px', fontWeight:'semi-bold', color:'#321154'}}>Rabbi {shul.shul.shul_rav}</Typography>
                        {shul.shul.shul_website ?<Typography>Link To Shul Website :  <a href={shul.shul.shul_website}>{shul.shul.shul_website}</a></Typography>
